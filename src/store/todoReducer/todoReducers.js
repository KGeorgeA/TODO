...
const todoReducers = (state = { allCompleted, filter, items }, action) => {
  switch (action.type) {
    ...
    case DELETE_TODO:
      return {
        ...state,
        allCompleted: state.items.length !== 1 ? state.allCompleted : false,
        items:
          state.items.length !== 1
            ? state.items.filter(
                (item) => parseInt(item.id) !== parseInt(action.id)
              )
            : [],
      };
    ...
    case TOGGLE_COMPLETED:
      let newItems = state.items.map((item) =>
        parseInt(item.id) === parseInt(action.id)
    ...
    case TOGGLE_ALL_COMPLETED:
      return {
        ...
        items: state.items.map((item) =>
          item.isCompleted === state.allCompleted
            ? { ...item, isCompleted: !item.isCompleted }
            : item
        ),
      };
